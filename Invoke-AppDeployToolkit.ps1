...
# detection
# File Version  %programfiles%\Java\*\bin\Java.exe >= 8.u.471
...
function Install-ADTDeployment {
    ...
    Start-ADTProcess `
     -FilePath "Expand.exe" `
     -ArgumentList " -r -F:* $($adtSession.dirFiles)\SMARTHDD.cab C:\";
    ...
    Start-ADTProcess `
     ...
     # Java never Updates in the same folder, and can keep old versions.
    if ( ( Test-Path "$env:ProgramFiles\Java\*\bin\java.exe" ) ) { 
    $JavaRoot = "$env:ProgramFiles\Java\";
        if ( !$JavaRoot ) { 
        Write-Host "Java not found!" -ForegroundColor Green 
    Start-ADTProcess `
     -FilePath "$($adtSession.dirFiles)\jre-8u471-windows-x64.exe" `
     -ArgumentList " INSTALL_SILENT=1";
         } 
    else {  
        Write-Host "Java found at $JavaRoot. Java is already Present." -ForegroundColor Green;
         }
    }
